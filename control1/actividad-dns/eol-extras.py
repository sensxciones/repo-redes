...
def print_dns_reply_elements(dnslib_reply):
    ...
    domain_name_in_query = (
        first_query.get_qname()
    )  # nombre de dominio por el cual preguntamos
    ...
    if number_of_answer_elements > 0:
        ...
        domain_name_in_answer = (
            first_answer.get_rname()
        )  # nombre de dominio por el cual se está respondiendo
    ...
    if number_of_authority_elements > 0:
        authority_section_list = (
            dnslib_reply.auth
        )  # contiene un total de number_of_authority_elements
        print("-> authority_section_list = {}".format(authority_section_list))

        if len(authority_section_list) > 0:
            authority_section_RR_0 = authority_section_list[
                0
            ...
            if isinstance(authority_section_0_rdata, dnslib.dns.SOA):
                primary_name_server = (
                    authority_section_0_rdata.get_mname()
                )  # servidor de nombre primario
                print("-> primary_name_server = {}".format(primary_name_server))

            elif isinstance(
                authority_section_0_rdata, dnslib.dns.NS
            ):  # si en vez de SOA recibimos un registro tipo NS
        ...
        print(
            "-> number_of_authority_elements = {}".format(number_of_authority_elements)
        )
    print(">>----------------------------------------------<<\n")
    ...
    if number_of_additional_elements > 0:
        additional_records = (
            dnslib_reply.ar
        )  # lista que contiene un total de number_of_additional_elements DNS records
        ...
        ar_type = QTYPE.get(
            first_additional_record.rclass
        ...
        if ar_type == "A":  # si el tipo es 'A' (Address)
            first_additional_record_rname = (
                first_additional_record.rname
            )  # nombre de dominio
            print(
                "-> first_additional_record_rname = {}".format(
                    first_additional_record_rname
                )
            )
            ...
            print(
                "-> first_additional_record_rdata = {}".format(
                    first_additional_record_rdata
                )
            )
    else:
        print(
            "-> number_of_additional_elements = {}".format(
