...
<body>
    ...
    <h3>El usuario {NOMBRE} ha bloqueado esta pagina</h3>
...
socket_A_address = ("172.20.144.24", 8000)  # direccion cliente (IP VM)
...
while True:
    ...
    socket_A, socket_A_address = (
        proxy_socket_A.accept()
    )  # socket_A es el socket que se comunica con el cliente
    ...
    # Separar puerto si viene en el Host
    if ":" in http_host:
        host_only = http_host.split(":")[0]
    else:
        host_only = http_host
    ...
    if is_blocked(http_path, data["blocked"]):
        ...
        proxy_socket_B = socket.socket(
            socket.AF_INET, socket.SOCK_STREAM
        )  #        print('Creando socket - Proxy -> Destino\n')
        socket_B_address = (ip_host, 80)  # direccion destino
        proxy_socket_B.connect(
            socket_B_address
        )  # Conecto el socket al address #        print('Conectado al servidor destino\n')

        # Reconstruyo el mensaje HTTP
        http_message_parsed["headers"]["X-ElQuePregunta"] = "PROOOXY"
        http_message = create_http_message(
            http_message_parsed
        ...
        proxy_socket_B.send(
            http_message.encode()
        )  # Envio el mensaje HTTP reconstruido al servidor destino
        ...
        # Si no, las reemplazo y armo nuevo mensaje HTTP
            ...
            censored_http_parsed_message = replace_body_http(
                response_raw_parsed, censored_html
            )
